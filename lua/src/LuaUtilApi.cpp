        ...
        .beginNamespace("sihd")
            .beginNamespace("util")
                ...
                .beginClass<Waitable>("Waitable")
                    ...
                    .addFunction("wait_for_loop", +[] (Waitable *self, time_t nano, uint32_t times) { return self->wait_for_loop(nano, times); })
...
void    LuaUtilApi::load_base(Vm & vm)
{
    ...
    luabridge::getGlobalNamespace(vm.lua_state())
        .beginNamespace("sihd")
            ...
            .beginNamespace("util")
                .beginNamespace("log")
                    .addFunction("emergency", +[] (const std::string & log) { logger.log(emergency, log); })
                    .addFunction("alert", +[] (const std::string & log) { logger.log(alert, log); })
                    ...
                    .addFunction("notice", +[] (const std::string & log) { logger.log(notice, log); })
