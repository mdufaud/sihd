...
class SysLogger: public ALogger
{
    public:
# if !defined(__SIHD_WINDOWS__)
        SysLogger(const std::string & progname, int facility = LOG_USER, int options = LOG_NDELAY | LOG_PID);
# else
        SysLogger(const std::string & progname, int facility = 0, int options = 0);
# endif
        virtual ~SysLogger();
