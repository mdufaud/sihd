...
class ObservableDelegate: Observable<T>
{
    public:
        ...
        // Allow passing the observable to anyone, but prevent the notify change that is now public
        class Protector
        {
            public:
                ...
                template <typename... Parameters>
                bool remove_observer_inside_notification(Parameters &&...args)
                {
                    return _delegate.remove_observer_inside_notification(std::forward<Parameters>(args)...);
                }
