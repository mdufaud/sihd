...
int main(int argc, char **argv)
{
    ...
    FilePoller fpoller("file-poller");

    fpoller.set_path(watch_path);
    fpoller.set_max_depth(result["depth"].as<size_t>());
    fpoller.set_polling_frequency(result["time"].as<double>());

    fpoller.add_observer(&handler);

    fpoller.set_start_synchronised(true);
    fpoller.start();
