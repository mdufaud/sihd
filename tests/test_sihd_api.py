...
class TestSihdApi(unittest.TestCase):
    ...
    def test_resources(self):
        ...
        self.assertTrue(sihd.path.get('test_sihd_api.py') is None)
        self.assertTrue(sihd.path.get('output') is None)
        sihd.path.add(sihd.path.get('tests'))
        filename = sihd.path.get('test_sihd_api.py')
        dirname = sihd.path.get('output')
        logger.info(filename)
        logger.info(dirname)
        self.assertTrue(filename)
        self.assertTrue(dirname)
        self.assertTrue(sihd.path.get_path('tests'))
        self.assertTrue(sihd.path.get_path('output') is None)
        self.assertTrue(sihd.path.get_dir('tests', 'output'))
        self.assertTrue(sihd.path.get_file('tests', 'test_sihd_api.py'))
        self.assertTrue(sihd.path.get_file('output') is None)
        self.assertTrue(sihd.path.add('wrong', 'path', 'sorry') is False)
        self.assertTrue(sihd.path.remove("nothing") is False)
        self.assertTrue(sihd.path.remove("tests"))
