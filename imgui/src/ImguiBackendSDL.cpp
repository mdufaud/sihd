...
bool ImguiBackendSDL::init_backend_opengl()
{
    ...
    _is_init = ImGui_ImplSDL3_InitForOpenGL(_sdl_window_ptr, _sdl_context_ptr);
    return _is_init;
}
...
bool ImguiBackendSDL::init_backend_dx()
{
    ...
    _is_init = ImGui_ImplSDL3_InitForD3D(_sdl_window_ptr);
    return _is_init;
}
...
void ImguiBackendSDL::terminate()
{
    if (_sdl_window_ptr != nullptr)
    {
        SDL_GL_DestroyContext(_sdl_context_ptr);
        _sdl_context_ptr = nullptr;
...
void *ImguiBackendSDL::windows_window()
{
    ...
    return (HWND)SDL_GetPointerProperty(SDL_GetWindowProperties(_sdl_window_ptr),
