...
// std::string get_error(zip_file_t *ptr)
// {
//     return zip_file_strerror(ptr);
// }
...
ZipFile::ZipFile(): _zip_ptr(nullptr), _current_zip_file_ptr(nullptr)
{
    this->no_encrypt();
}
...
bool ZipFile::set_password(std::string_view password)
{
    ...
    {
        SIHD_LOG(error, "ZipFile: could not set password: {}", get_error(_zip_ptr));
        return false;
    }
    return true;
}
...
void ZipFile::discard()
{
    if (_zip_ptr != nullptr)
    {
        close_zip_file_and_null(&_current_zip_file_ptr);
...
bool ZipFile::close()
{
    ...
    if (_zip_ptr != nullptr)
    {
        ...
        if (!ret)
            SIHD_LOG(error, "ZipFile: could not close zip file: {}", get_error(_zip_ptr));
        _zip_ptr = nullptr;
    }
    _current_zip_entry = ZipEntry {};
    _buf.clear();
...
std::string_view ZipFile::archive_comment() const
{
    if (_zip_ptr == nullptr)
        return "";
    int comment_length;
    const char *archive_comment = zip_get_archive_comment(_zip_ptr, &comment_length, 0);
...
bool ZipFile::comment_archive(std::string_view comment)
{
    if (_zip_ptr == nullptr)
        return false;
    const bool success = zip_set_archive_comment(_zip_ptr, comment.data(), comment.size()) == 0;
    if (!success)
    {
        SIHD_LOG(error, "ZipFile: could not write zip archive commentary: {}", get_error(_zip_ptr));
    }
    return success;
}
...
bool ZipFile::set_archive_readonly(bool active)
{
    if (_zip_ptr == nullptr)
        return false;
    const bool success = zip_set_archive_flag(_zip_ptr, ZIP_AFL_RDONLY, (int)active) == 0;
    if (!success)
    {
        SIHD_LOG(error,
...
{
    return zip_get_num_entries(_zip_ptr, ZIP_FL_UNCHANGED);
}

ssize_t ZipFile::count_entries() const
{
    return zip_get_num_entries(_zip_ptr, 0);
}
...
bool ZipFile::load_entry(size_t idx)
{
    ...
    const bool success = zip_stat_index(_zip_ptr, idx, 0, &zip_entry) == 0;
    if (success)
    {
    ...
    {
        SIHD_LOG(error, "ZipFile: could not read entry '{}': {}", idx, get_error(_zip_ptr));
    }
    return success;
}
...
bool ZipFile::load_entry(std::string_view name)
{
    ...
    zip_stat_t zip_entry;
    zip_stat_init(&zip_entry);
    const bool success = zip_stat(_zip_ptr, name.data(), 0, &zip_entry) == 0;
    if (success)
    {
        save_entry(&zip_entry, _current_zip_entry);
    }
    ...
    {
        SIHD_LOG(error, "ZipFile: could not read entry '{}': {}", name, get_error(_zip_ptr));
    }
    return success;
}
...
{
    return _zip_ptr != nullptr && _current_zip_entry.index >= 0;
}
...
std::string_view ZipFile::entry_comment() const
{
    ...
    const char *comment = zip_file_get_comment(_zip_ptr, _current_zip_entry.index, &comment_length, ZIP_FL_ENC_UTF_8);
...
ssize_t ZipFile::read_entry(std::string_view password)
{
    ...
    ssize_t ret = zip_fread(_current_zip_file_ptr, _buf.data(), _buf.capacity());
    if (ret < 0)
    {
...
bool ZipFile::unchange_entry()
{
    if (this->is_entry_loaded() == false)
        return false;
    if (zip_unchange(_zip_ptr, _current_zip_entry.index) < 0)
    {
        SIHD_LOG(error,
...
bool ZipFile::remove_entry()
{
    if (this->is_entry_loaded() == false)
        return false;
    if (zip_delete(_zip_ptr, _current_zip_entry.index) < 0)
    {
        SIHD_LOG(error,
...
bool ZipFile::rename_entry(std::string_view new_name)
{
    if (this->is_entry_loaded() == false)
        return false;
    if (zip_file_rename(_zip_ptr, _current_zip_entry.index, new_name.data(), ZIP_FL_ENC_UTF_8) < 0)
    {
        SIHD_LOG(error,
...
bool ZipFile::modify_entry_time(sihd::util::Timestamp new_timestamp)
{
    if (this->is_entry_loaded() == false)
        return false;
    if (zip_file_set_mtime(_zip_ptr, _current_zip_entry.index, new_timestamp.seconds(), 0) < 0)
    {
        SIHD_LOG(error,
                 "ZipFile: could not rename entry '{}': {}",
                 ZIP_ENTRY_NAME_OR_INDEX(_current_zip_entry),
...
bool ZipFile::comment_entry(std::string_view comment)
{
    if (this->is_entry_loaded() == false)
        return false;
    if (zip_file_set_comment(_zip_ptr, _current_zip_entry.index, comment.data(), comment.size(), ZIP_FL_ENC_UTF_8) < 0)
    {
        SIHD_LOG(error,
                 "ZipFile: could not rename entry '{}': {}",
                 ZIP_ENTRY_NAME_OR_INDEX(_current_zip_entry),
...
bool ZipFile::encrypt_entry(std::string_view password)
{
    ...
    const bool ret
        = zip_file_set_encryption(_zip_ptr, _current_zip_entry.index, _encryption_method, password.data()) == 0;
    if (!ret)
        SIHD_LOG(error,
...
bool ZipFile::replace_entry(sihd::util::ArrCharView view)
{
    ...
    const bool success = zip_file_replace(_zip_ptr, _current_zip_entry.index, source, ZIP_FL_ENC_UTF_8) == 0;
    if (!success)
    {
        zip_source_free(source);
        SIHD_LOG(error,
...
bool ZipFile::add_file(std::string_view name, sihd::util::ArrCharView view)
{
    ...
    if (source == nullptr)
    {
        SIHD_LOG(error, "ZipFile: could not create source buffer for entry '{}' to zip: {}", name, get_error(_zip_ptr));
        return false;
    }
    const ssize_t file_idx = add_source(_zip_ptr, name, source);
    return file_idx >= 0;
}
...
bool ZipFile::add_from_fs(std::string_view name, std::string_view path)
{
    if (_zip_ptr == nullptr)
        return false;
    if (fs::is_dir(path))
        return this->add_dir_from_fs(path, name);
    else if (fs::is_file(path))
        return this->add_file_from_fs(path, name);
    return false;
}
...
bool ZipFile::add_dir_from_fs(std::string_view name, std::string_view path)
{
    ...
    {
        SIHD_LOG(error, "ZipFile: could not add directory '{}' to zip: {}", name, get_error(_zip_ptr));
        return false;
    }
    std::vector<std::string> children = fs::children(path);
    for (std::string_view child : children)
    {
        if (this->add_from_fs(fs::combine(path, child), fs::combine(name, child)) == false)
            return false;
    }
    return true;
}
...
bool ZipFile::add_file_from_fs(std::string_view name, std::string_view path)
{
    ...
    if (source == nullptr)
    {
        SIHD_LOG(error, "ZipFile: could not create source file for entry '{}' to zip: {}", name, get_error(_zip_ptr));
        return false;
    }
    return add_source(_zip_ptr, name, source);
}

bool ZipFile::dump_entry_to_fs(std::string_view path, std::string_view password)
{
    if (_zip_ptr == nullptr || !this->is_entry_loaded())
        return false;

    if (this->is_entry_directory())
    {
        const bool success = fs::make_directory(path.data(), 0640);
        if (!success)
            SIHD_LOG(error, "ZipFile: could not write directory entry '{}' to: {}", _current_zip_entry.name, path);
        return success;
    }
    ...
    if (file.is_open())
    {
        ...
        {
            ret = file.write(_buf.data(), read) > 0;
            if (!ret)
            {
                SIHD_LOG(error, "ZipFile: could not write file entry '{}' to: {}", _current_zip_entry.name, path);
                close_zip_file_and_null(&_current_zip_file_ptr);
            }
        }
        file.close();
